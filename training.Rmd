...
p <- Sys.getenv("PATH")
Sys.setenv(PATH=paste0(p,";C:\\Users\\chocho\\Miniconda3\\condabin"))
```
...
if (!("forestatrisk" %in% conda_list()$name)) {
	...
	conda_install("forestatrisk", "forestatrisk", pip=TRUE, pip_ignore_installed=FALSE)
...
```{r comp_diff}
if (!file.exists(glue("output/diff2050_icar_glm.tif"))) {
	far$r_diffproj(inputA=glue("output/proj2050_icar.tif"),
				   inputB=glue("output/proj2050_glm.tif"),
				   output_file=glue("output/diff2050_icar_glm.tif"),
				   blk_rows=128L)
}
```
...
```{r plot_diff}
if (!file.exists(glue("output/diff2050_icar_glm.png"))) {
    far$plot$differences(glue("output/diff2050_{comp[i]}.tif"),
                         borders="data/mada/mada38s.shp",
                         output_file=glue("output/diff2050_icar_glm.png"))
}
```
